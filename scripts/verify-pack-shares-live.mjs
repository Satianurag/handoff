...
try{
 ...
 const url=new URL('/records/file',process.env.CONVEX_URL.replace('.convex.cloud','.convex.site'));url.searchParams.set('householdId',householdId);url.searchParams.set('title','Synthetic share source');const upload=await fetch(url,{method:'POST',headers:{Authorization:`Bearer ${b.tokens.token}`,'Content-Type':'application/pdf','X-Filename':'synthetic-care-follow-up.pdf'},body:await readFile(new URL('./fixtures/synthetic-care-follow-up.pdf',import.meta.url))});assert(upload.ok,'Source upload failed');const {recordId}=await upload.json();
